...
export const Avatar = ({id, src, size = 32}: Props) => (
  <div className={`Avatar ${src ? 'has-image' : ''}`}>
    ...
    <style jsx>{`
      ...
      .Avatar__inner {
        border-radius: 50%;
        color: #5b55f7;
        background: ${`hsl(${hashStr(id) % 360}, 60%, 98%)`};
        border: 1px solid ${`hsl(${hashStr(id) % 360}, 60%, 92%)`};
        display: inline-flex;
        align-items: center;
      ...
      .Avatar__inner > svg {
        fill: ${`hsl(${hashStr(id) % 360}, 60%, 82%)`};
        width: ${size / 2.666}px;
        height: ${size / 2.666}px;
      }
      .Avatar__inner > img {
        max-width: 100%;
        height: auto;
