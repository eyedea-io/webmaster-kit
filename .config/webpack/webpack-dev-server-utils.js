...
function prepareUrls(protocol, host, port) {
  const formatUrl = ({hostname, protocol, port}) =>
    url.format({
      protocol,
      hostname,
      port,
  ...
  const prettyPrintUrl = ({hostname, protocol, port}) =>
    url.format({
      protocol,
      hostname,
  ...
  const hostname = isUnspecifiedHost ? 'localhost' : host
  const localUrlForTerminal = prettyPrintUrl({hostname, protocol, port})
  const bundleUrlForTermianl = prettyPrintUrl({hostname, protocol, port: 8888})
  const localUrlForBrowser = formatUrl({hostname, protocol, port})

  return {
    bundleUrlForTermianl,
    localUrlForTerminal,
    localUrlForBrowser,
  }
}
...
function printInstructions(appName, urls, useYarn) {
  console.log()
  console.log(`  ${chalk.bold('Server:')}   ${urls.localUrlForTerminal}`)
  console.log(`  ${chalk.bold('Bundle:')}   ${urls.bundleUrlForTermianl}`)
